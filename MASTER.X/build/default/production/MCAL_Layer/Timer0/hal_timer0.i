...
# 39 "MCAL_Layer/Timer0/../GPIO/hal_gpio.h"
typedef enum{
    GPIO_LOW = 0,
    GPIO_HIGH
}logic_t;
...
# 1 "MCAL_Layer/Timer0/../Interrupt/mcal_interrupt_conifg.h" 1
# 15 "MCAL_Layer/Timer0/../Interrupt/mcal_interrupt_conifg.h"
# 1 "MCAL_Layer/Timer0/../Interrupt/mcal_interrupt_gen_cfg.h" 1
# 15 "MCAL_Layer/Timer0/../Interrupt/mcal_interrupt_conifg.h" 2
# 57 "MCAL_Layer/Timer0/../Interrupt/mcal_interrupt_conifg.h"
typedef enum{
...
# 52 "MCAL_Layer/Timer0/hal_timer0.c"
        (T0CONbits.TMR0ON = 1);
    }
    return status;
}
...
# 105 "MCAL_Layer/Timer0/hal_timer0.c"
static void Timer0_Prescaler(const timer0_t *_timer){
    if(0 == _timer->prescaler_mode){
        (T0CONbits.PSA = 0);
        T0CONbits.T0PS = _timer->prescaler_value;
    }
    else if(1 == _timer->prescaler_mode){
        (T0CONbits.PSA = 1);
    }
    else{ }
}
...
static void Timer0_Mode_Select(const timer0_t *_timer){
    if(0 == _timer->timer0_mode){
        ...
        (T0CONbits.T0CS = 1);
        if(0 == _timer->counter_edge){
            (T0CONbits.T0SE = 0);
        }
        else if(1 == _timer->counter_edge){
            (T0CONbits.T0SE = 1);
        }
        else{ }
    }
...
static void Timer0_Register_Size(const timer0_t *_timer){
    if(0 == _timer->register_size){
        (T0CONbits.T08BIT = 1);
    }
    else if(1 == _timer->register_size){
        (T0CONbits.T08BIT = 0);
    }
    else{ }
}
